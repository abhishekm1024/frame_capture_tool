...
**Last successful build:** user-M17 — `./gradlew assembleDebug` + `./gradlew test` BUILD SUCCESSFUL; new CameraX threading regression suite 4/4 pass
...
| M12 (user-M14) | Release preparation | ✅ Complete | Optional release-signing wired via `keystore.properties` (gitignored; template tracked as `keystore.properties.template`); `debug.versionNameSuffix = "-debug"`; manifest now declares `android:dataExtractionRules`; dead `tools:targetApi="31"` attribute dropped; `proguard-rules.pro` re-annotated with per-block rationale + `-dontwarn kotlinx.coroutines.debug.**`; hot-path `Log.d("MAX_FRAMES_IN_FLIGHT")` removed from `CameraXFrameSource`; `DebugProbesKt.bin` excluded from packaging. `assembleRelease` produces `app-release-unsigned.apk` (4,443,957 bytes, R8 minified) when no keystore present; produces signed APK when `keystore.properties` is supplied. |
| user-M15 | Runtime bug fix: scan-path error classification (C-25) | ✅ Complete | `ScanErrorClassifier` introduced in `feature-scan`; replaces the two unconditional `ERROR_CAMERA_UNAVAILABLE` mappings in `ScanViewModel.bindCameraIfReady` with type-aware routing. New string `ERROR_CAMERA_INIT` covers generic CameraX bind failures so the Samsung S23 case (permission granted, CameraService streaming, CameraX bind raises an exception) no longer shows the misleading "Please check permissions." message. The three spec-mandated strings (screen_specs §5.3) are unchanged byte-for-byte. 11 new unit tests pin the classifier behavior. |
| user-M16 | Runtime diagnostics: structured logging across scan init path (C-26) | ✅ Complete | `ScanLog` helper added (`internal` to `:features:feature-scan`); emits one-line breadcrumbs and full cause-chain error logs. `ScanViewModel` now logs every phase (`startArSession`, every `ArSessionEvent`, `bindCameraIfReady` entry/wait/start) and previously-unlogged `ArSessionEvent.Error` cause. `CameraXFrameSource` logs at provider acquisition, selector resolution, preview build, `bindToLifecycle` start/success/failure (failure path logs class + message + unwrapped cause chain before propagating). `ArRepositoryImpl.startSession` logs every emission (`createSession.*`, `bindLifecycle.done`, `emit.ready`). Silent `runCatching { provider.unbindAll() }` masks tightened to `runCatching {…}.onFailure { Log.w(…) }`. `feature-scan` gains `testOptions.unitTests.isReturnDefaultValues = true` so unit-test JVM tolerates the new `android.util.Log` calls. 7 new ScanLogTest cases pin the formatter behavior (kv pairs, null tolerance, cycle safety, cause-chain join). |
| user-M17 | Runtime bug fix: CameraX threading violation (C-27) | ✅ Complete | `CameraXFrameSource.asFlow` previously built `ImageAnalysis`, `Preview` (+ `surfaceProvider` setter at line 74), and called `setAnalyzer` before entering `withContext(dispatchers.main)`. ScanViewModel collects this flow from `viewModelScope.launch(dispatchers.io)`, so the callbackFlow body ran on IO — CameraX's `Threads.checkMainThread()` raised `IllegalStateException: Not in application's main thread` from inside `Preview.Builder().build().also { it.surfaceProvider = provider }`. The phase=startCapture.failed log added in user-M16 exposed it. **Fix:** the entire CameraX-touching block (build `ImageAnalysis`, build `Preview` + `surfaceProvider` assignment, `setAnalyzer`, `unbindAll`, `bindToLifecycle`) moved inside the existing `withContext(dispatchers.main)`. `stop()` also moved `clearAnalyzer()` into its `withContext(dispatchers.main)` for symmetry. **Test:** 4 new `CameraXFrameSourceThreadingTest` cases — (a) control-test proving the fixture detects an IO-leaked flow body, (b) `withContext(dispatchers.main)` actually switches Thread when collector is on IO, (c) **structural source-text guard** asserting that `Preview.Builder()`, `.setAnalyzer(`, and `bindToLifecycle(` all live strictly after `withContext(dispatchers.main)` opens in `asFlow` (so a future refactor cannot reintroduce the bug), (d) constructor smoke check with mocked `Context`. |
...
- Actual plan milestones completed: M0, M1-A/B/C, M2 (data-camera), M3 (data-ar), M4 (data-firebase), M5-A/B/C (UI features), M6 (feature-scan, user-M9), plus plan-M7 (feature-upload, user-M6), plan-M8 integration (user-M10), build infra (user-M11/M12), production hardening audit (user-M13), release preparation (user-M14), scan-path error classification fix (user-M15), runtime-diagnostics structured logging (user-M16), CameraX threading-violation fix (user-M17).
- **All feature + data + integration + release-config work complete.** Remaining: plan-M9 device-level testing + owner deployment of `google-services.json` debug client + `firebase/storage.rules`.
...
| C-23 | Debug `applicationIdSuffix = ".debug"` would enable side-by-side debug+release install, but requires registering `com.sfm.scanner.debug` as a second client in Firebase (and a corresponding `google-services.json` entry). Owner-supplied step; suffix is currently disabled in `app/build.gradle.kts` with a comment pointing here. The `versionNameSuffix = "-debug"` change is in place so the debug build is still distinguishable in `Settings → Apps`. | `:app/build.gradle.kts`, Firebase console | Owner-side: add debug client to Firebase project, restore `applicationIdSuffix`. |
| C-24 | Release-signing wired via optional `keystore.properties` (gitignored). Template tracked as `keystore.properties.template`. Without the file, `assembleRelease` produces `app-release-unsigned.apk` (regression-safe). To produce a signed APK an owner must generate a keystore (`keytool -genkey -v -keystore sfm-scanner-release.jks -keyalg RSA -keysize 2048 -validity 10000 -alias sfm-scanner-release`) and populate `keystore.properties`. Keystore itself never enters version control (`*.jks` gitignored). | `keystore.properties.template`, `app/build.gradle.kts` | Owner-side: generate keystore + populate `keystore.properties`. |
| C-25 | ~~`ScanViewModel.bindCameraIfReady` collapsed every exception out of `cameraRepository.startCapture(...)` to `ScanStrings.ERROR_CAMERA_UNAVAILABLE`, whose verbatim spec text ends with "Please check permissions." On the reported Samsung S23 (permission granted, CameraService streaming) the user saw a misleading permission error for a CameraX bind failure that had nothing to do with permission.~~ | ~~`features/feature-scan/.../ScanViewModel.kt` lines 308 & 316~~ | ✅ Resolved in user-M15. `ScanErrorClassifier` routes `CameraUnavailableException(CAMERA_DISABLED)` → permission error (kept), ARCore exceptions → AR-unsupported, everything else (bind / use-case-combo / surface-in-use) → new `ERROR_CAMERA_INIT` ("Camera could not be initialized. Please close other camera apps and try again."). 11 new unit tests including an explicit S23-scenario regression case. AR depth degradation already handled by `DepthAvailabilityChecker` (verified in audit; no change needed). |
| C-26 | ~~Scan-screen init-path failures were logged as `"... failed: ${e.message}"` with `e.message` frequently `null` for CameraX/ARCore exceptions, so the actual exception class and cause chain were buried in the stack trace and not greppable. `ArSessionEvent.Error` was not logged at all — any post-`CameraShared` AR failure was invisible. `runCatching { provider.unbindAll() }` swallowed cleanup errors silently. On the Samsung S23 the upstream bug-fix M15 corrected the display string but the real root cause remained undiagnosable from logs.~~ | ~~`features/feature-scan/.../ScanViewModel.kt`, `data/data-ar/.../ArRepositoryImpl.kt`, `data/data-camera/.../CameraXFrameSource.kt`~~ | ✅ Resolved in user-M16. Structured `ScanLog` helper plus phase breadcrumbs at every init step. Failure logs include class, message, and full unwrapped cause chain. Silent `runCatching` masks tightened to log on failure. Reproduction on the S23 now produces a greppable timeline ending in `phase=…failed | class=… | message=… | cause-chain=…`. |
| C-27 | ~~`CameraXFrameSource.asFlow` built `ImageAnalysis`, `Preview` (incl. `surfaceProvider` assignment), and called `setAnalyzer` from the callbackFlow body — which runs on the *collector's* dispatcher. `ScanViewModel.bindCameraIfReady` collects from `viewModelScope.launch(dispatchers.io)`, so all this CameraX construction executed on IO. CameraX 1.4 enforces Main via `Threads.checkMainThread()` and throws `IllegalStateException: Not in application's main thread`. Camera permission was fine, Camera 0 opened, privacy indicator went green — bug only surfaced once user-M16's `phase=startCapture.failed` log printed the real exception class.~~ | ~~`data/data-camera/.../CameraXFrameSource.kt`~~ | ✅ Resolved in user-M17. Moved every CameraX construction call (build `ImageAnalysis`, build `Preview`+`surfaceProvider`, `setAnalyzer`, `unbindAll`, `bindToLifecycle`) into the existing `withContext(dispatchers.main)` block. `stop()` also moved `clearAnalyzer()` onto Main for symmetry. Architecture, public API, and feature behavior unchanged. 4 new unit tests in `CameraXFrameSourceThreadingTest` pin the dispatcher contract — including a static source-text guard that fails the build if `Preview.Builder()`, `.setAnalyzer(`, or `bindToLifecycle(` ever leave the Main-thread block. |
...
| `:data:data-camera` | `TimestampGateTest` (8), `JpegFrameEncoderNv21Test` (6), `CameraXFrameSourceThreadingTest` (4, user-M17, pins C-27) | `ResolutionPickerTest` (6), `JpegFrameEncoderTest` (4) |
...
| `:features:feature-scan` | `StopScanUseCaseTest` (5 tests), `ScanViewModelTest` (10 tests), `FrameWriterTest` (5 tests), `ScanErrorClassifierTest` (11 tests, user-M15, pins C-25), `ScanLogTest` (7 tests, user-M16, pins C-26 formatter), `MainDispatcherRule` | `PackageSessionUseCaseTest` (5 tests, ZIP integration), `ScanScreenTest` (5 tests) |
| `:features:feature-scan`, `:features:feature-upload` test totals | above | above |

**Last executed (user-M17):** `./gradlew assembleDebug` — BUILD SUCCESSFUL; `./gradlew test` — BUILD SUCCESSFUL across all 11 modules including new `CameraXFrameSourceThreadingTest` (4/4 green). `./gradlew assembleRelease` last green at user-M14 (unchanged this milestone). `./gradlew connectedAndroidTest` still not run (needs physical device + ARCore + Firebase reachability).
...
**Implement plan-M9: Device-level Testing & owner-side deployment.**

State as of user-M14:
- `./gradlew assembleDebug` and `./gradlew assembleRelease` both green.
- `./gradlew test` runs all unit tests green.
- M11 hardening (user-M13) and M12 release preparation (user-M14) complete.
- Residual owner-side items: C-23 (debug-variant Firebase client), C-24 (release keystore + `keystore.properties`), R-15 deployment.

Key deliverables for plan-M9 (per implementation_plan.md §M9):
1. Owner-side prep: generate release keystore + populate `keystore.properties` (C-24); add `com.sfm.scanner.debug` client to Firebase project + re-enable `applicationIdSuffix` in `app/build.gradle.kts` (C-23); deploy `firebase/storage.rules` (R-15).
